...
@section('title', 'Edit Blocked RegID')

@section('content')
<div class="card">
    <div class="card-header">
        <h1 class="card-title">Edit Blocked RegID</h1>
    </div>

    <form action="{{ route('admin.blocked-regids.update', $blockedRegid) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label class="form-label">Registration ID (RegID) *</label>
            <input type="text" name="regid" class="form-control" value="{{ old('regid', $blockedRegid->regid) }}" required placeholder="Enter RegID to block">
            @error('regid')
        ...
        <div class="form-group">
            ...
            <textarea name="reason" class="form-control" rows="3" placeholder="Reason for blocking this RegID">{{ old('reason', $blockedRegid->reason) }}</textarea>
            @error('reason')
                <div style="color: #ef4444; margin-top: 5px; font-size: 12px;">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <p style="font-size: 14px; color: #6b7280; margin-bottom: 10px;">Select locations where this RegID will be blocked:</p>
        ...
        <div style="margin-top: 30px;">
            <button type="submit" class="btn btn-primary">Update Blocked RegID</button>
