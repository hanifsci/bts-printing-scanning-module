...
@section('title', 'Create Post Data API Configuration')

@section('content')
<div class="card">
    <div class="card-header">
        <h1 class="card-title">Create Post Data API Configuration</h1>
    </div>
    ...
    <form action="{{ route('admin.api-configurations.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" class="form-control" value="{{ old('name') }}" required placeholder="e.g., External Registration API">
            @error('name')
        ...
        <div class="form-group">
            ...
            <textarea name="description" class="form-control" rows="3" placeholder="Optional description of this API">{{ old('description') }}</textarea>
            @error('description')
                <div style="color: #ef4444; margin-top: 5px; font-size: 12px;">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <p style="font-size: 13px; color: #6b7280; margin-bottom: 15px;">
                ...
                Leave blank to use default field names (regid, category, name, etc.)
            </p>
            
            <div id="field-mappings-container">
                <!-- Default mappings will be used if none are added -->
            </div>
            
            <button type="button" id="add-mapping" class="btn btn-secondary" style="margin-top: 10px;">Add Custom Field Mapping</button>
            <p style="font-size: 12px; color: #6b7280; margin-top: 10px;">
                <strong>Default mappings:</strong> If you don't add custom mappings, the API will use default field names: regid, category, name, email, mobile, designation, company, country, state, city, additional1-5, receipt_number, is_lunch_allowed
            </p>
        </div>

        <div style="margin-top: 30px;">
            <button type="submit" class="btn btn-primary">Create Post Data API Configuration</button>
            <a href="{{ route('admin.api-configurations.index') }}" class="btn btn-secondary" style="margin-left: 10px;">Cancel</a>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addBtn.addEventListener('click', function() {
        ...
        row.innerHTML = `
            <div>
                <input type="text" name="api_field[]" class="form-control api-field-input" placeholder="API Field Name" required>
                <small style="color: #6b7280;">API sends this field</small>
            </div>
            <div>
                <select name="db_column[]" class="form-control db-column-select" required>
                    ${dbOptions}
                </select>
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        rows.forEach((row) => {
            const apiFieldInput = row.querySelector('.api-field-input');
            const dbColumnSelect = row.querySelector('.db-column-select');
        ...
        // Remove the array inputs
        const apiFieldInputs = this.querySelectorAll('input[name="api_field[]"]');
        const dbColumnInputs = this.querySelectorAll('select[name="db_column[]"]');
        apiFieldInputs.forEach(input => input.remove());
        dbColumnInputs.forEach(input => input.remove());
