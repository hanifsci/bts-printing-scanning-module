...
@section('title', 'Edit Bypassed RegID')

@section('content')
<div class="card">
    <div class="card-header">
        <h1 class="card-title">Edit Bypassed RegID</h1>
    </div>

    <form action="{{ route('admin.bypassed-regids.update', $bypassedRegid) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label class="form-label">Registration ID (RegID) *</label>
            <input type="text" name="regid" class="form-control" value="{{ old('regid', $bypassedRegid->regid) }}" required placeholder="Enter RegID to bypass">
            @error('regid')
        ...
        <div class="form-group">
            ...
            <textarea name="reason" class="form-control" rows="3" placeholder="Reason for bypassing this RegID (required)" required>{{ old('reason', $bypassedRegid->reason) }}</textarea>
            @error('reason')
                <div style="color: #ef4444; margin-top: 5px; font-size: 12px;">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="max_uses" class="form-control" value="{{ old('max_uses', $bypassedRegid->max_uses) }}" min="1" placeholder="Leave empty for 1 use">
            @error('max_uses')
                <div style="color: #ef4444; margin-top: 5px; font-size: 12px;">{{ $message }}</div>
            @enderror
        ...
        <div class="form-group">
            ...
            <p style="font-size: 14px; color: #6b7280; margin-bottom: 10px;">Select locations where this RegID will be bypassed:</p>
        ...
        <div style="margin-top: 30px;">
            <button type="submit" class="btn btn-primary">Update Bypassed RegID</button>
